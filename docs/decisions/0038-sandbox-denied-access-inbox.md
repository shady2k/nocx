...
On macOS the backend mints a 128-bit per-launch token, renders it into the Seatbelt deny-default message, and starts `/usr/bin/log stream --style json` with a predicate containing only that token. Only matching `Sandbox: <program>(<pid>) deny ... file-* <absolute-path>` entries are parsed.
