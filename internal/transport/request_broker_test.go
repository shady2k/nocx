...
func (h *harness) waitForRenderers(t *testing.T, n int) {
	t.Helper()
	testwait.WaitForTimeout(t, "renderer registered", wantWithin, func() bool {
		h.srv.mu.Lock()
...
func TestBroker_ConnectionLossOfOneRendererLeavesRequestAnswerableByAnother(t *testing.T) {
	...
	_ = connA.Close()
	testwait.WaitForTimeout(t, "A's loss processed", wantWithin, func() bool {
		h.srv.mu.Lock()
		defer h.srv.mu.Unlock()
...
func TestBroker_FailedDeliveryIsNotARecipient(t *testing.T) {
	...
	testwait.WaitForTimeout(t, "pending request registered", wantWithin, func() bool {
