...
// (nocx-dkawo.14).
//
// Every worker call was already asserted where it lives, and the sequence they
...
// calls, over the real worker record, opening real sessions in real panes.
//
...
func streamModelToolCall(w http.ResponseWriter, name, args string) {
	frame := map[string]any{
		"id": "chatcmpl-worker", "object": "chat.completion.chunk", "model": "probe-model",
...
// A MODEL SPAWNS THREE WORKERS, WAITS ON THE WORKER, AND CLOSES THEM.
//
...
func TestAModelSpawnsThreeWorkersWaitsOnTheWorkersAndClosesThem(t *testing.T) {
	...
	provider.then = func(last string) (scriptedTurn, bool) {
		...
		id := toClose[0]
		toClose = toClose[1:]
		return scriptedTurn{tool: "workers.close", args: `{"worker":"` + id + `"}`}, true
	}
	...
	go func() {
		waittest.WaitFor(t, "the first worker to be live", func() bool {
			...
			return herr == nil && len(held) >= 1 && held[0].State == workers.StateLive
		})
		held, _ := w.workerStore.HeldBy(ctx, string(w.coordinator))
	...
	held, err := w.workerStore.HeldBy(ctx, string(w.coordinator))
	if err != nil {
	...
	for _, p := range held {
		waittest.WaitFor(t, "every worker to reach a terminal state", func() bool {
			stored, serr := w.workerStore.Participant(ctx, p.ID)
...
func liveWorkerIDs(requestBody string) []string {
	...
	for _, p := range answer.Participants {
		if p.State == string(workers.StateLive) {
			out = append(out, p.ID)
		}
	}
	return out
...
// autonomousWorkerGrant mints the authority a coordinator run holds: every
...
func autonomousWorkerGrant(sessionID, env string) content.Grant {
