# ADR-0040 — The per-tab filesystem sandbox launches an interactive shell

- **Status:** Accepted
- **Date:** 2026-08-18
- **Related:** ADR-0036, ADR-0037, ADR-0038, ADR-0039, AD-7, AD-8, AD-11, `nocx-y46q.18`
- **Supersedes:** ADR-0038 launch-target decision and every fixed-OpenCode clause it added to ADR-0036/AD-11; ADR-0038's private runtime-tree bootstrap placement remains adopted
...
3. The private bootstrap repair from ADR-0038 remains: the composition root creates the per-session runtime tree first; bash rcfile or zsh ZDOTDIR is written under its writable `tmp/`; the native backend adopts the same tree; every failure and close path removes it idempotently.
...
- ADR-0036's action and consequence text reads `Sandboxed shell…`, not `Sandboxed opencode…`; its policy includes the canonical shell and native enforcement dependencies, not a fixed agent executable.
- ADR-0038 decisions 1, 3, 4, and 5 concerning the fixed OpenCode intent are superseded. Its decision 2 concerning artifact placement remains adopted, minus the agent tail.
- ADR-0039 decision 8 no longer includes a fixed OpenCode executable among backend-derived roots. All directory-class and policy-conflict decisions remain unchanged.
