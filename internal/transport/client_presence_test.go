...
// PrimePresence answers "nobody yet" the instant the observer is wired,
// with no listener up and no connection ever made (nocx-xn63t.6.10).
//
// Start's own trailing notePresence (ws.go) reports the same zero, but only
// once the listener exists — which is AFTER internal/app's restart
// reconciliation has already run (app.go sequences it before Transport.Start
// so a client cannot ask sessions.live mid-pass). A reconciled session whose
// credential lives in a sealed vault needs EnsureUnsealed to SUSPEND for the
// client Start is about to accept, and the vault can only tell "nobody yet"
// from "nobody ever" (internal/vault/presence.go's clientsKnown) once
// SOMETHING has reported a count — which, without this method, is nothing at
// all during that whole pass. PrimePresence is what a composition root calls
// before running anything that might block on the vault, so the tri-state
// reads "wired, and zero" rather than "never wired" at the one moment it
// used to read the latter by omission.
func TestPrimePresence_ReportsZeroWithNoListenerAndNoConnection(t *testing.T) {
	rec := &recordingPresence{}
	ws := NewWSServer(log.NewSlogAdapter(nil), newRegWithStub(log.NewSlogAdapter(nil)),
		WithVaultLifecycle(newFakeVaultLifecycle()),
		WithClientPresence(rec))
	...
	n, ok := rec.last()
	if !ok {
		t.Fatal("PrimePresence told the observer nothing")
	}
	if n != 0 {
		t.Fatalf("PrimePresence reported %d clients, want 0", n)
	}
}

// A server built with no presence observer at all — every WSServer a test
// that is not about presence constructs — must not panic for having nobody
// to tell. notePresence already guards this for Start's own call; this is
// the same guard exercised through the new entry point.
func TestPrimePresence_NoObserverWiredIsHarmless(t *testing.T) {
	ws := NewWSServer(log.NewSlogAdapter(nil), newRegWithStub(log.NewSlogAdapter(nil)),
		WithVaultLifecycle(newFakeVaultLifecycle()))

	ws.PrimePresence() // must not panic
}
