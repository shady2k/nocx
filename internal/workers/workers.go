// Package worker holds the record the backend keeps for a worker of agents: who
...
//
// Two records, and neither implies the other (A1 of the worker authority
...
//
// A worker call carries NO authority the session does not already have (A12).
//
...
//   - The worker endpoint is a real unix socket, so the kernel stamps each peer.
//     internal/peerpin pins the enrolled root as (pid, startTime) and walks a
...
// ParticipantID names one participant within one worker. It is minted by the
...
// coordinator may do to a worker is a Delegation, and a worker in a worker with
...
const (
	...
	// exactly one per worker in this slice (D15: one worker first, not three).
...
// worker-specific authority record would be a second vocabulary for one concept
...
var (
	...
	ErrNoSuchParticipant = errors.New("worker: no such participant")
	// ErrBoundExceeded means the worker already holds as many participants as
	...
	// one.
	ErrStaleEvidence = errors.New("worker: evidence names another incarnation")
	...
	ErrTerminal = errors.New("worker: participant is already terminal")
