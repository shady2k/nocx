...
func sayTo(t *testing.T, h *harness, from, to ReaderID, body string) Message {
	t.Helper()
	m, err := h.reg.Say(context.Background(), testGroup, from, to, body)
	if err != nil {
...
func TestAMessageNobodyFetchedIsReportedAsCommittedAndNotAsDelivered(t *testing.T) {
	...
	owed, err := h.reg.Undelivered(ctx, testGroup)
	if err != nil {
...
// worker's mailbox and cannot be written to.
func TestMailIsCheckedAgainstMembershipAndNothingElse(t *testing.T) {
	ctx := context.Background()
	h := newHarnessBound(t, 5)
	mustRegister(t, h)

	if _, err := h.reg.Say(ctx, testGroup, coordBox, "p-not-in-this-worker", "hello"); !errors.Is(err, ErrNotAMember) {
		t.Fatalf("writing to a stranger returned %v, want ErrNotAMember", err)
	}
	if _, err := h.reg.Say(ctx, testGroup, "sess-stranger", workerBox, "hello"); !errors.Is(err, ErrNotAMember) {
...
func TestAMessageIsBoundedAndAnEmptyOneIsRefused(t *testing.T) {
	...
	if _, err := h.reg.Say(ctx, testGroup, coordBox, workerBox, ""); !errors.Is(err, ErrEmptyMessage) {
		t.Fatalf("an empty message returned %v, want ErrEmptyMessage", err)
	}
	big := strings.Repeat("x", MaxMessageBytes+1)
	if _, err := h.reg.Say(ctx, testGroup, coordBox, workerBox, big); !errors.Is(err, ErrMessageTooLarge) {
...
// one repeat while losing a message costs the worker.
