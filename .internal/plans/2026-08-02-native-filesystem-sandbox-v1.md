...
`docs/decisions/0019-native-per-tab-filesystem-sandbox.md`.
...
- Probe: missing binary → `sandbox-exec-unavailable`; non-zero exit → `probe-failed`;
  success cached.
- Real release-artefact enforcement (runs in CI on macos-latest against the packaged `.app`,
  `make sandbox-smoke-macos`): the common assertions from slice 2 against canonical temp
  fixtures (create/truncate/rename in writable roots, read system executables, sentinel
  outside roots unreadable, symlink/subprocess/renamed-path bypass attempts fail, post-launch
  hard-link creation denied), plus the same pre-existing-hard-link limitation documented as
  on Linux (Seatbelt is also path-hierarchy-based; exact parity is `[ASSUMPTION]` until the
  suite runs), PTY interactivity, and `sandbox-exec` availability. Gated with the repo's
  explicit native-integration convention: skipped **loudly** with a named reason when the
  `.app` is absent; never silently skipped, and the release gate fails if the gate did not
  run.

**Smallest focused command:** `make sandbox-smoke-macos` on a macOS runner with a packaged
`.app`; locally `go test -count=1 ./internal/sandbox/ -run Profile`.
...
- Landlock ABI floor (3) and cap (8) appear verbatim in: spec §8.1, ADR-0019, this plan's
