...
func TestParticipantBySessionNamesTheWorkerRunningInIt(t *testing.T) {
	...
	if err := store.EnsureGroup(ctx, ID("coordinator-session"), "coordinator-session"); err != nil {
		t.Fatalf("ensure worker: %v", err)
	}
	p := Participant{
		ID:           ParticipantID("worker-1"),
		Group:        ID("coordinator-session"),
		Role:         RoleWorker,
		State:        StatePrepared,
...
func TestParticipantBySessionNeverMatchesTheEmptySession(t *testing.T) {
	...
	if err := store.CommitPrepared(ctx, Participant{
		ID: ParticipantID("worker-1"), Group: ID("coordinator-session"),
...
func TestParticipantBySessionPrefersTheLiveIncarnation(t *testing.T) {
	...
	for _, seed := range []struct {
		...
		if err := store.CommitPrepared(ctx, Participant{
			ID: seed.id, Group: ID("coordinator-session"), Role: RoleWorker,
