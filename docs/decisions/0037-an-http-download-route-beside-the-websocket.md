...
- **Heading corrected 2026-08-28 (`nocx-yjvg5`):** this file's heading read `ADR-0040`, a number [another record](0040-a-block-is-a-node-in-an-ordered-tree.md) already holds, and it cited its sibling as `ADR-0039` for the same reason. The filenames were never in conflict, so they won. A citation of "ADR-0040" written before that date may mean this document.
