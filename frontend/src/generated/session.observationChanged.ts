...
export interface SessionObservationChanged {
  ...
  agent: string
  /**
   * The THIRD facet, beside the state rather than inside it: whether a pane that is WORKING has moved lately (nocx-tnx44). 'moving' means its transcript changed within the stall threshold, or that the question cannot be asked — a pane that is not working, and a pane whose rule reads no transcript at all, both answer 'moving' rather than claiming a stagnation nothing measured. 'stalled' means the pane is working and its transcript has not changed for longer than the threshold, and it is the case this field exists for: a hung agent's spinner keeps spinning, so the chrome around a frozen transcript animates forever and nothing about the screen itself says the turn is stuck. It rides BESIDE the state and may never become one — a pane that is working and stalled is still, in every listing and every comparison, a pane whose state is 'working' — because this is a comparison between two readings of one screen taken over time, and the driver holds no state between frames on purpose.
   */
  progress: 'moving' | 'stalled'
  /**
   * What the screen is inviting, from a CLOSED set. 'free_text' means an input box is on screen and waiting, and it is the only state nocx may type into. 'permission_choice' means the agent raised a question of its own — a tool approval, or the folder-trust question it asks before it will start in a directory — and is waiting on a human, so answering it answers the agent. 'modal_choice' means a menu the agent did not raise is up — a user-opened one such as /model. 'working' means work is in progress: a turn in flight, or a background agent the main turn may be blocked on, which Claude Code shows with the input box still live and no spinner at all. 'error' means the TUI's OWN error is on screen — the API unreachable, overloaded, or the quota gone — which the agent draws as chrome. It is not 'unknown', because unknown is treated as busy and busy tells a person to leave it alone when the truth is that this will not resolve itself; and it is not 'permission_choice', because there is nothing to answer. An error the agent PRINTED into its transcript before going back to waiting is 'free_text', deliberately: on the screen that is indistinguishable from idle because it IS idle. 'unknown' means the driver could not positively identify the state, or the agent has no driver; every caller treats it as busy, because refusal is the containment here rather than mitigation — a mistimed keystroke does not merely fail to arrive, it answers whatever modal is on screen. 'exited' is a fact about the PROCESS and is never read off the screen; no driver returns it.
   */
  state:
