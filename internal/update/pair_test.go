...
func TestInProcessCoordinatorProbe_ReportsTheBuildItWasGiven(t *testing.T) {
	p := NewInProcessCoordinatorProbe("0.2.0", "abc123")
	got, err := p.AnsweringCoordinator(context.Background())
	if err != nil {
		t.Fatalf("the in-process probe cannot fail: %v", err)
	}
	if got.Version != "0.2.0" || got.Commit != "abc123" {
		t.Errorf("got %+v, want {0.2.0 abc123}", got)
	}
}

// A build whose backend is its own process certifies its own update —
// the paired success for the refusals above, and the check that the
// pair rule does not break the arrangement that has no daemon at all.
func TestReportHealthy_InProcessBackend_Finalises(t *testing.T) {
	installPath, jp := pendingUpdate(t, "0.1.0", "0.2.0")
	...
	u := NewUpdater(UpdaterConfig{
		...
		CurrentVersion: "0.2.0",
		InstallPath:    installPath,
		Coordinator:    NewInProcessCoordinatorProbe("0.2.0", "newsha"),
	})

	if err := u.ReportHealthy(context.Background()); err != nil {
		t.Fatalf("an in-process backend at the installed version must certify: %v", err)
	}
	if _, err := os.Stat(jp); !os.IsNotExist(err) {
		t.Error("journal not deleted after a certified health report")
	}
