...
func TestWaveAuthorizerRefusesASecondLiveCallerForTheSameSession(t *testing.T) {
	...
	} else if got, want := err.Error(), "session already has a wave caller"; got != want {
		t.Fatalf("second caller error = %q, want %q", got, want)
	}
}

func TestWaveAuthorizerReleasesSlotAfterConnectionAndInFlightCallSettle(t *testing.T) {
	reg, sess, grid := openWaveAuthSession(t)
	const ownedPID = 4242
	if err := reg.RecordOwnedProcessPID(sess.ID(), ownedPID); err != nil {
		t.Fatalf("record owned process pid: %v", err)
	...
	} else if got, want := admitErr.Error(), "session already has a wave caller"; got != want {
