-- SCHEMA 13, VERBATIM, and it is the shape ONE BELOW THE LADDER'S FIRST RUNG.
--
-- This is `schemaV1` as it stood at 1ccad4de582309988ab0f8f954dbd076eafb21c5,
-- the last commit holding `const schemaVersion = 13`, lifted unmodified. It is
-- the companion to schema_v14.sql and it exists for the boundary rather than
-- for the happy path: the ladder starts at 14, so 13 is the largest version
-- this build must REFUSE, and a fixture invented for that test would prove
-- nothing about a shape anybody actually shipped.
--
-- WHY A REAL 13 AND NOT A SYNTHETIC LOW NUMBER. Its table NAMES are the ones
-- this build knows — workspaces, tabs, panes, entries, grant_scopes, all of
-- them — while its COLUMNS are not: `entries` here has no `source` column at
-- all and its `kind` CHECK admits 'agent' where 15 admits 'ask' and 'frame'.
-- That is precisely the file the unknown-table gate could never catch, because
-- the gate compared names and the names match completely. A refusal that works
-- on `command_history` from schema 1 and not on this is a refusal that does not
-- work.
--
-- IT IS FROZEN, for the same reason schema_v14.sql is: a fixture edited to keep
-- up with the current schema stops being a fixture.
