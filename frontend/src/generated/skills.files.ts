...
export interface SkillsFiles {
  ...
  /**
   * Every file under the skill's directory, slash-separated and relative to it, SKILL.md first and the rest sorted — the order skills.preview uses for the same list, so the file the person came for is never found among its references. Symlinks are absent: their bytes live somewhere else and the read path refuses them, so naming one would be a row that can only fail to open. Never null and never empty; a skill with no support files is ["SKILL.md"].
   *
   * @minItems 1
   */
  files: [string, ...string[]]
