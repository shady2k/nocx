...
**Quality gates & CI.** Enforced from commit #1: both Go and TypeScript are gated the same way — format, lint, and test. Go uses `golangci-lint` and `gofumpt`; the frontend is held to the same bar. The per-commit gate is the `.githooks/pre-commit` hook, mirrored by `make ci`; GitHub Actions (`ci.yml`) validates `release/**` branches and manual dispatch, and is *called* by `release.yml` on a version tag so a release gates on a green suite. `ci.yml` no longer has its own tag trigger — with `release.yml` calling it, keeping one would run the whole suite twice per release. Not running on every PR is a deliberate temporary choice (tracked in beads) to avoid burning macOS runner time before the MVP has something worth guarding. Tests are mandatory (TDD) for every language.
