...
export interface AgentAccessList {
  ...
  answers: {
    ...
    /**
     * The MACHINE this answer is about (nocx-50w7p.16). A person's yes admits an executable ON ONE MACHINE: the same path and the same bytes on two hosts, or under two accounts on one host, is a different answer to give, so the machine is part of what the durable answer is keyed by. DERIVED BY THE BACKEND from the session's own route — its kind, its host, the account the connection authenticated as, and the host key it was accepted under — and never from a probe, a caller, or anything the agent says about itself. Four facts rather than one composed string, because the renderer is what words them: a composed value would make every surface parse a key whose grammar has one owner in the backend, the same rule this contract already states for `workspace`. The exact same object is declared in host.request.schema.json and in both agentAccess schemas; internal/transport's TestTheMachineShapeIsDeclaredOnce holds the three copies byte-identical.
     */
    machine: {
      /**
       * Which machine: `local` is the machine the coordinator runs on, `ssh` one reached over a connection the backend authenticated. A closed set the backend derives; a client never invents one, and only the revoke path may echo one back.
       */
      kind: 'local' | 'ssh'
      /**
       * ssh only: the host as dialed. Required exactly when kind is ssh — a machine nobody can name is an answer for no machine, so the backend refuses to key one.
       */
      host?: string
      /**
       * ssh only: the account the connection authenticated as. Two accounts on one host are two machines, because the agent being admitted runs as one of them.
       */
      account?: string
      /**
       * ssh only: the SHA-256 fingerprint of the host key the connection was accepted under, in the spelling every host-key error and every known_hosts line carries. It is IN the answer's identity for the reason ADR-0023 gives for helper consent: a machine whose key changed is a different answer to give, so the stored yes stops matching and the question is asked again.
       */
      hostKey?: string
    }
  }[]
