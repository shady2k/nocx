...
func (m *effectKernel) escalate(ctx context.Context, decl agenttools.Tool, callID, rawArgs string, args map[string]any) error {
...
func (m *effectKernel) escalateClassifier(ctx context.Context, decl agenttools.Tool, callID, rawArgs string, ask *ApprovalRequest, fact *classifierFact) error {
	ap := m.proposal(decl.Name, callID, rawArgs)
	var entryID string
	if m.ledger != nil {
...
func (k *effectKernel) Invoke(ctx context.Context, name, callID, rawArgs string) (string, error) {
	...
	outcome, refusal := k.decide(decl, args)
	switch outcome {
	case policyRefuse:
	...
	case policyAsk:
		...
		return "", k.escalate(ctx, decl, callID, rawArgs, args)
	}
	...
	if k.classifier != nil && !k.proposalApproved(decl.Name, callID, rawArgs) {
		...
		if ask != nil {
			return "", k.escalateClassifier(ctx, decl, callID, rawArgs, ask, fact)
		}
		classifierFact = fact
	}
