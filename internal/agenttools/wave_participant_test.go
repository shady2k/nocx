...
func TestWaveParticipantNamesOnlyItsOwnMailbox(t *testing.T) {
	p := NewWaveParticipant("worker-1")
...
// WaveCoordinator does: the dispatcher's type switch is what proves the
// distinction exhaustive, and a nil arriving here means that switch was
// bypassed — which must fail as a refusal, not as a crash.
func TestWaveParticipantNilNamesNothing(t *testing.T) {
	var p *WaveParticipant
...
func TestWaveCapabilitiesAreTwoTypes(t *testing.T) {
	var coordinator Capability = NewWaveCoordinator("session-1", nil)
	var participant Capability = NewWaveParticipant("worker-1")
	if _, ok := coordinator.(*WaveParticipant); ok {
		t.Fatal("a coordinator capability satisfies *WaveParticipant")
	}
	if _, ok := participant.(*WaveCoordinator); ok {
		t.Fatal("a participant capability satisfies *WaveCoordinator")
	}
}

// narrowWaveParticipant takes the id from the run context and never from the
...
func TestNarrowWaveParticipantTakesTheIDFromTheRunContext(t *testing.T) {
	capability, err := narrowWaveParticipant(
		content.Grant{}, nil, RunContext{Session: "worker-session", Participant: "worker-1"},
	)
	if err != nil {
		t.Fatalf("narrow: %v", err)
	}
	p, ok := capability.(*WaveParticipant)
	if !ok {
		t.Fatalf("narrow returned %T, want *WaveParticipant", capability)
	}
...
func TestNarrowWaveParticipantRefusesARunThatIsNoParticipant(t *testing.T) {
	if _, err := narrowWaveParticipant(
