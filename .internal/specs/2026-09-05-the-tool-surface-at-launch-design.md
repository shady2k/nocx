...
| **D3**  | **Use a launch-owned config file, and do NOT use `--strict-mcp-config`.** Claude receives `--mcp-config "$launchDir/mcp.json"` naming one server, `nocx`. The person's own MCP servers stay loaded; ours is ADDED to them. **Corrected 2026-09-06 (`nocx-rowqt.2`): D3 as first written contradicted the owner's constraint in §3.1, which predates it.**                                                                                                                       | This is not a change of mind and was never agreed: §3.1 states the owner's constraint as "Invasive at launch is welcome; editing anyone's config files is not", and switching off the servers a person configured for themselves is overriding their configuration whether or not a file is written. D3 as first written required `--strict-mcp-config` on the argument that omitting it "would let unrelated configured providers enter an orchestrated launch". Those providers are the person's own tools, which they configured for themselves, and nocx has no business switching them off in its own panes: that is intervening in a user's configuration, not protecting a launch. **Measured 2026-09-06 against Claude Code 2.1.260:** with `--mcp-config <file>` and no strict flag, a scratch-scoped user server and the supplied server were BOTH connected and the model listed both tools (`mcp__user-probe__user_probe_tool` and `mcp__nocx-probe__wave_holdings`). Our fence does not depend on strict mode either way: the endpoint checks peer credentials and the process-tree pin however many providers are running beside it. `--strict-mcp-config` keeps ONE legitimate use, in the `D10` conformance check, where the point is to measure our surface and not the developer's. Writing `.mcp.json` or calling `claude mcp add` remains rejected, for the original reason: it contaminates a file the user owns. |
...
```
claude --mcp-config "$launchDir/mcp.json" <original arguments>
```
...
3. `--strict-mcp-config` is NOT used in the product path (D3 as amended), so the person's
   own servers start alongside ours; it prevents the invocation from starting the project's or user's
