-- schema_v18.sql — the schemaV1 constant lifted VERBATIM out of the tree at
-- the commit where `const schemaVersion` held 18 (78b2bff4, the stage branch's
-- head at the time of the pin; 18 widened executions.termination_reason and
-- added no table since 17). Frozen fixture for the migration ladder's shape
-- pins, per the discipline schema_migrate_released_test.go documents: never
-- regenerated from the current schema. schema 19 adds worker_checkouts on top
