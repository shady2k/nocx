...
- **Date:** 2026-08-02
- **Touches:** ADR-0019 (this decision), AD-7/AD-8 (session model and composition-root wiring),
...
- Parse only the selected root's `.git` file (a `gitdir: <path>` line). Resolve and
  canonicalize `gitdir`, add it read-write, and expose it in `writableRoots`.
- Do **not** recursively search parent directories; do **not** shell out to Git. A malformed
  or missing `.git` file yields no extra root (and is not an error).
...
1. `filepath.Abs(workspace)` — reject on error.
2. `filepath.EvalSymlinks(abs)` — reject on error (missing path or broken symlink).
3. `os.Stat(canonical)` — must be a directory; reject otherwise.
...
- `Request{Workspace string}` — canonicalized by the caller (transport), never by the backend.
...
- `CommandSpec{Path string; Args []string; Dir string; Env []string}` — the ordinary shell
  command, exactly as `pty.NewLocal` builds it today.
...
- Serialize the bounded policy (JSON, validated by the same parser as the parent) into an
  **unlinked** mode-`0600` file descriptor; pass it through `ExtraFiles`.
- A second pipe carries the readiness/error handshake (parent reads one byte:
  `0` = enforced, `1` = setup failure with the typed reason on the pipe; bounded read
  timeout).
...
| I9 | Registration only after enforcement readiness: session registry entry, ring, and tab exist only after the readiness handshake succeeds. |
...
The same common assertions against canonical temp fixtures, plus PTY interactivity and
`sandbox-exec` availability. Gated with the repository's existing explicit native-integration
convention (devharness-backed e2e; a gated run is skipped loudly with a named reason, never
silently — `e2e/vault.spec.ts` pattern, research report §4.5).

### 11.3 Regression (slice 6)

- Ordinary local and SSH tabs remain behaviorally unchanged (open/resize/close/exit).
- A failed sandbox setup creates no registered session/tab.
...
| Git worktree `.git` present | common dir added RW and listed in `writableRoots`; malformed `.git` → no extra root |
