...
| `claude-2.1.272-wrapped-echo`       | `session-message-wrapped-echo.script` | the idle box (`30000`), then a one-paragraph task bracketed-pasted into it and left unsubmitted, drawn over FOUR content rows (`50000`) — the box whose reading `session.message` confirms its own paste on (nocx-xn63t.4.5)                                                   |
...
## Recorded for nocx-xn63t.4.5, on Claude Code 2.1.272

One capture, `claude-2.1.272-wrapped-echo`, taken 2026-09-17 on this machine through
`.claude/skills/nocx-detection-verify/record.sh` with its own new script,
`scripts/session-message-wrapped-echo.script`. `claude --version` reported `2.1.272 (Claude
Code)`, and the endpoint was `http://127.0.0.1:9` — a dead port on purpose: the script's last
step pastes a task and presses no Enter, so no turn ever starts and no model has to answer.

What it exists for: no capture in the corpus had an input box taller than the TWO content
rows an older Claude's wrap happened to fill, and `nocx-xn63t.4.5` is the defect that hid
...
`\x1b[200~…\x1b[201~` escapes), because that is what the product sends: a `text` atom crosses
as `sessionruntime.IntentKindPaste` and the runtime encodes it with ghostty's own paste
encoder, which wraps it in the brackets whenever the program has enabled bracketed paste —
Claude has. Claude answers a one-line paste with the TEXT itself and not its
`[Pasted text #N +M lines]` placeholder, which is the other half of what this capture settles:
the placeholder form `boxContainsEcho` also accepts is for a paste that carries newlines.

**The wrap is Claude's own repaint, not the terminal's.** The bytes for the pasted box are
`❯␣Please ESC[10Gread … ESC[113Gthan\r ESC[2C ESC[1B one …`: every word is painted at an
explicit column, and the row break is `\r`, a two-column move and a down-one. So the
emulator's own `Wrap`/`Continuation` flags are FALSE for all four rows — there is no soft
wrap to read — and **the space the break consumed is written nowhere**: between `than` and
`one` the frame holds no space at all, which is why the reading restores one rather than
only dropping the `\n` it used to join with.
