...
/**
 * Result of the git.open JSON-RPC method: the outcome table of resolving a repository for a session (design §5.1, remote-helper design §6), plus the binding that names it for every later git.* call. sessionId appears exactly once on the wire — here — and the authorisation is connState's, not the global session registry's (D15): a connection that learned another connection's session id gets a refusal, not a repository. state is the discriminator: every outcome is a RESULT state, never a JSON-RPC error, because each one is something the panel can render (not a repository, git absent, git too old, no verified cwd, an unsupported platform, a failed deploy, an exec the host refused, a version-mismatched helper). A machine with no helper-tier answer at all is not a state here (ADR-0068): git.open never asks, so that case answers the JSON-RPC not-available error, naming the connection setting that would change it, exactly like raw or a denied answer. The first status rides this result — otherwise every open is two round trips and a guaranteed frame of empty lists — and is present exactly when the inline read succeeded.
 */
export interface GitOpenResult {
  /**
   * The open outcome (remote-helper design §6). noCwd is decided by the transport from the session's origin before the repo factory is invoked; the refusal states (unsupportedPlatform, deployFailed, execForbidden, helperVersionMismatch) are decided by the helper selection and dial; the factory itself answers ok, notARepository, gitUnavailable or gitTooOld.
   */
  state:
    | 'ok'
    | 'notARepository'
