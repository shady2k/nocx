...
// newParticipant is one prepared worker of the shared test workers.
...
// newSeededStore is an empty record with the worker every test here needs.
func newSeededStore(t *testing.T) *MemoryStore {
	t.Helper()
	s := NewMemoryStore()
	if err := s.EnsureGroup(context.Background(), testGroup, coordSession); err != nil {
...
func TestAParticipantIsReadBackAsItWasWritten(t *testing.T) {
	...
	if got.Group != testGroup || got.Role != RoleWorker || !got.RegisteredAt.Equal(p.RegisteredAt) {
...
// delegation and a taken-over worker is still in the worker's holdings. The coordinator's
...
func TestHoldingsAreAnsweredBySessionAndSurviveALostDelegation(t *testing.T) {
	...
	elsewhere.Group = ID("worker-2")
	if err := s.EnsureGroup(ctx, "worker-2", "sess-another-coordinator"); err != nil {
		t.Fatalf("ensure the other worker: %v", err)
	}
	...
	if len(held) != 1 || held[0].ID != p.ID {
		t.Fatalf("held = %v, want exactly %q — a takeover removed a worker from its own worker", held, p.ID)
	}
}
...
func TestOnlyOpenParticipantsAreListed(t *testing.T) {
	...
	// And an open participant of ANOTHER worker is not this worker's: the
	// reservation counts what one coordinator started, so a second worker's
	...
	got, err := s.NonTerminal(ctx, testGroup)
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	if len(got) != 2 {
		t.Fatalf("listed %d participants, want the 2 of this worker that are open: %v", len(got), got)
	}
	for _, p := range got {
		if p.State.Terminal() {
		...
		if p.Group != testGroup {
			t.Fatalf("participant %q of worker %q listed under %q", p.ID, p.Group, testGroup)
		}
	}
}

// The coordinator session of a worker is its identity: a second spawn into a
// worker that exists must not move every participant to a controller that never
// spawned them.
func TestASecondEnsureDoesNotMoveAGroupToAnotherCoordinator(t *testing.T) {
	ctx := context.Background()
	s := newSeededStore(t)
	if err := s.EnsureGroup(ctx, testGroup, "sess-someone-else"); err != nil {
		t.Fatalf("second ensure: %v", err)
	}
	got, err := s.CoordinatorSession(ctx, testGroup)
	if err != nil {
		t.Fatalf("coordinator session: %v", err)
	}
	if got != coordSession {
		t.Fatalf("coordinator = %q, want %q — a second spawn reassigned the worker", got, coordSession)
	}
}
...
func TestWhatTheRecordDoesNotHoldIsRefusedByName(t *testing.T) {
	...
	if _, err := s.CoordinatorSession(ctx, "worker-nobody"); !errors.Is(err, ErrNoSuchParticipant) {
...
func commitTo(t *testing.T, s *MemoryStore, to, from ReaderID, body string) Message {
	...
	m, err := s.Commit(context.Background(), Message{
		Group: testGroup, Recipient: to, Sender: from, Body: body,
...
func TestUndeliveredAsksTheRecipientsOwnCursor(t *testing.T) {
	...
	open, err := s.Undelivered(ctx, testGroup)
	if err != nil {
...
func TestTheRecordIsSafeUnderConcurrentUse(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func() {
			defer wg.Done()
			if _, err := s.NonTerminal(ctx, testGroup); err != nil {
			...
			if _, err := s.Undelivered(ctx, testGroup); err != nil {
