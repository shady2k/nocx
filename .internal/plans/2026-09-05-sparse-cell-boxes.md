...
describe('serializeRange cell boxes', () => {
  ...
  it('отвергает вердикт, не равный колонкам ячейки', () => {
    ...
    ).toContain('<span class="term-cell" data-cols="2">漢</span>')
  })
...
Примечание для исполнителя: `DEFAULT_SNAPSHOT` здесь годится, потому что классификатору из атрибутов нужны только `bold` и `italic`, а они от темы не зависят. Если тест «видит ровно те ячейки» покажет расхождение — искать причину в нём, а не подгонять тест.
