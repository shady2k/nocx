...
**Bead:** `nocx-r7dle`  
**Decision:** [ADR-0039](../../decisions/0039-explicit-home-grants-project-into-isolated-home.md)  
...
The host HOME is the last `HOME=` value in the original `CommandSpec.Env`, matching the existing last-value environment convention. If absent, the backend uses `os.UserHomeDir`. The selected value passes through `canonicalExistingDir` before `sandboxEnv` rewrites the child environment. A present but unusable HOME, or a failed fallback, is a path-free `SetupError` (`-32012`), not an empty projection set.
