...
func TestRegisterMintsUnguessableIDs(t *testing.T) {
	reg := New()
	id1, err := reg.Register(newStubProvider(), "s1", "", nil)
	if err != nil {
		t.Fatal(err)
	}
	id2, err := reg.Register(newStubProvider(), "s1", "", nil)
	if err != nil {
		t.Fatal(err)
	}
	if id1 == id2 {
	...
	if _, err := reg.Register(nil, "s1", "", nil); err == nil {
...
func TestAcquireRequiresOwnership(t *testing.T) {
	reg := New()
	id, err := reg.Register(newStubProvider(), "s1", "", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBindingIdentityAccessors(t *testing.T) {
	reg := New()
	id, err := reg.Register(newStubProvider(), "s1", "v1:abc", nil)
	if err != nil {
		t.Fatal(err)
	}
	h, release, err := reg.Acquire(id, owner("s1"))
...
func TestCloseSessionClosesOnlyThatSessionsBindings(t *testing.T) {
	...
	pB := newStubProvider()
	pC := newStubProvider()
	idA, err := reg.Register(pA, "s1", "", nil)
	if err != nil {
		t.Fatal(err)
	}
	idB, err := reg.Register(pB, "s1", "", nil)
	if err != nil {
		t.Fatal(err)
	}
	idC, err := reg.Register(pC, "s2", "", nil)
	if err != nil {
		t.Fatal(err)
	}
	reg.CloseSession("s1")
...
func TestRegistryConcurrentUse(t *testing.T) {
	...
	ids := make([]string, 8)
	for i := range ids {
		id, err := reg.Register(newStubProvider(), session.ID("s1"), "", nil)
		if err != nil {
			t.Fatal(err)
		}
		ids[i] = id
...
func TestCloseReportsProviderAndWatchErrors(t *testing.T) {
	...
	reg2 := New()
	id2, err := reg2.Register(newStubProvider(), "s1", "", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRegisterRejectsTypedNilProvider(t *testing.T) {
	...
	if _, err := reg.Register(p, "s1", "", nil); err == nil {
