...
- **Status:** evidence ledger for `.internal/specs/2026-08-02-native-filesystem-sandbox-design.md`,
  ADR-0019, and `.internal/plans/2026-08-02-native-filesystem-sandbox-v1.md`
...
- **Boundary for nocx:** nocx reuses *architectural lessons and behavior*, never AGPL code.
...
- **Consequence:** nocx's fail-closed launch is a *deliberate divergence* from termic, not a
...
| 6 | Worktree `.git` `gitdir:` parsing for the Git common dir | **Borrow** (behavior) | termic `parent_git_dir_for_worktree()` ~L1948 == design spec §5.4; both parse only the selected root's `.git` file, never shell out |
...
| 11 | Proposed Docker/OCI path | **Reject** | `docs/plans/docker-sandbox/*` is proposal-only in termic and container runtime is not selected for nocx (ADR-0019 "Alternatives") |
...
- `sandbox-exec` remains present on the *latest* macOS release — not executed on this host
