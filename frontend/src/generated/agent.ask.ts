...
export interface AgentAsk {
  ...
  runId: number
  /**
   * The TURN's entry id in the ledger — ONE id and not two (nocx-4em1z). A turn is a block: the question is the entry's intent and the answer is its body, exactly as a command line and its output are. The streamed deltas append to it (agent.runDelta's entryId), the flow renders it as the block, and a restore reads it back. The renderer needs it BEFORE the first delta so a run that fails before any text still has a block to terminalize.
   */
  entryId: string
  /**
  ...
  state:
    ...
    | 'interrupted'
  /**
   * The turn's ingest_seq — commit order in the one ledger (ADR-0019 §2: the stable total order for paging, never causality).
   */
  ingestSeq: number
  /**
