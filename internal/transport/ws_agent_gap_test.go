...
func TestAgentAsk_SlowConsumerBoundsTheDeltaPath(t *testing.T) {
	...
	waittest.WaitForTimeout(t, fmt.Sprintf("%d delta frames written", writtenDeltas), 10*time.Second, func() bool {
	...
	close(client.release)
	waittest.WaitForTimeout(t, "terminal runState on the wire", 10*time.Second, func() bool {
