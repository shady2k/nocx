...
// THE 17→18 RUNG: executions.termination_reason learns `answer-revoked`
// (nocx-4yjwk.7).
//
...
// aReleasedSchema17Database writes the file a shipped schema 17 build would
...
// stamp.
//
// It is built from testdata/schema_v17.sql rather than from the current
...
func aReleasedSchema17Database(t *testing.T, path string) {
	t.Helper()
	statements := []string{
	...
	// One execution per reason schema 17 admitted, each with a distinctive
	// executor string, so a copy that transposed the columns is visible.
	for i, reason := range reasonsSchema17Knew {
...
// reasonsSchema17Knew is the CHECK's vocabulary at 17, frozen. It is written
...
var reasonsSchema17Knew = []string{
	"completed", "failed", "timeout", "transport-gone", "user-killed",
	"agent-declined", "interrupted", "inactivity", "output-budget",
}

// THE HEADLINE: a real schema 17 database with executions in it comes across
// the rung with every row intact, and the widened CHECK then accepts the
// reason 16 could not store.
func TestAReleasedSchema17DatabaseMigratesAndKeepsEveryExecution(t *testing.T) {
	path := filepath.Join(t.TempDir(), "content.db")
	aReleasedSchema17Database(t, path)

	db, err := Open(context.Background(), Config{
		Path: path, Key: schemaTestKey(), Budget: testBudgetInternal(), Logger: log.NewSlogAdapter(nil),
	})
	if err != nil {
		t.Fatalf("Open over a released schema 17 database: %v — a version behind must migrate, not refuse", err)
	}
	...
	if n := rawCount(t, path, `SELECT count(*) FROM executions`); n != len(reasonsSchema17Knew)+1 {
		t.Fatalf("executions holds %d rows after the rung, want %d", n, len(reasonsSchema17Knew)+1)
	}
	...
	// exists: the value schema 17 refused now inserts.
...
func TestADatabaseStampedAboveSchema17IsStillRefused(t *testing.T) {
	...
	if n := rawCount(t, path, `SELECT count(*) FROM executions`); n != len(reasonsSchema17Knew)+1 {
		t.Fatalf("executions holds %d rows after the refusal, want %d — the refusal destroyed data", n, len(reasonsSchema17Knew)+1)
	}
}
...
func assertTheRungItselfLeavesAnUnfinishedRunUntouched(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("walk the ladder over a schema 17 database: %v", err)
	}
