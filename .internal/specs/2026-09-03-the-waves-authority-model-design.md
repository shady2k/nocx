...
| A12 | **Until participant authenticity has a mechanism, a wave call carries NO authority the session does not already have**                                                                                                                                                                                   | Claiming the enrolment pin exists. It does not — see §5 — and a model that assumes "only the enrolled tree may speak for the participant" would be resting on a property no mechanism provides — `nocx-aqz7o` showed the candidate bearer was not one, and its fix was to DELETE the field                                                                                                                                                                                               |
...
| `SO_PEERCRED` / `LOCAL_PEERCRED` | **Implemented on the wave endpoint (amended 2026-09-06, `nocx-rowqt.8`)** | Still true of the LIFECYCLE channel, which is a `socketpair`: no `connect(2)`, no per-peer credential to stamp, and loopback TCP has none by construction. It stopped being true of the SECOND caller's channel — `internal/waveendpoint` listens on a real unix socket, so the kernel stamps every peer and `D13`'s "OS identity from peer credentials" has its carrier there. Two channels, two answers                                                                                                                                                                               |
| `pidfd` / start-time pin         | **Built (amended 2026-09-06, `nocx-rowqt.8`)**                            | `internal/wavepin` pins the enrolled root as `(pid, startTime)` and walks a caller's ancestry to it, per OS: `/proc/<pid>/stat` parsed after the last `)` on Linux, `kern.proc.pid` on darwin. A pid reused after the root exits fails closed because the start time no longer matches, and a broken chain is refused rather than admitted. Still no `pidfd`; the start-time comparison is what makes the pid non-reusable                                                                                                                                                              |
