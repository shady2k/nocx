...
//
// A lease times the SUPERVISOR, so it runs during a worker that is entirely
...
type Fact struct {
	...
	// rather than looked up when the alarm fires: by then the worker may hold
	...
	// of a log line.
	Wake WakeOutcome
	// AlsoOwed is how many OTHER facts of the same worker were undispatched at
	// the moment this one reached the human. It exists because escalation
	// coalesces per worker: five workers finishing while the coordinator is
...
type workerState struct {
	...
	changed chan struct{}
	// woken records that a wake was DELIVERED for this worker and the
	...
	woken bool
	// escalated records that the human has been told about this worker and has
...
// record of another worker's facts.
func (b *Backstop) Entered(ctx context.Context, f Fact) {
	...
	b.stats.Judgement++
	b.announce(f.Group)
	ws := b.workerOf(f.Group)
	ws.owed++
	// One wake per worker per undispatched run. The coordinator's answer to a
	// wake is workers.holdings, which returns EVERYTHING its session holds — so
	...
	if alreadyWoken {
		...
		if cur, still := b.open[key]; still {
			cur.Wake = WakeOutcome{
				Reason: "the coordinator was already woken for this worker and has not fetched since",
			}
		}
		b.mu.Unlock()
		b.log.Debug("worker: the coordinator is already awake for this worker",
			"participant", string(f.Participant), "worker", string(f.Group))
		return
	}
...
func (b *Backstop) attempt(ctx context.Context, f Fact) WakeOutcome {
	if b.wake == nil {
		b.log.Error("worker: a fact needs judgement and this backend has nothing to wake a coordinator with",
			"participant", string(f.Participant), "worker", string(f.Group), "kind", string(f.Kind))
		return WakeOutcome{Reason: "this backend has no way to type into a pane"}
	}
	out := b.wake.Wake(ctx, f.CoordinatorSession, wakeText(f))
	out.Attempted = true
	if out.Delivered {
		b.log.Info("worker: the coordinator was woken",
		...
		b.log.Info("worker: the coordinator was not woken, and the fact stays undispatched",
...
func wakeText(f Fact) string {
	...
	return fmt.Sprintf("nocx: your worker %s %s and is %s. Call workers.holdings to see what your session holds.",
...
// Whether a card appears is the worker's decision, not the fact's.
func (b *Backstop) due(key factKey) {
	...
	if repeat {
		// The human has a card for this worker already and has not seen it
		...
		b.log.Info("worker: another fact went undispatched, and the human already has this worker",
			"participant", string(snapshot.Participant), "worker", string(snapshot.Group),
			"also_owed", snapshot.AlsoOwed)
		return
	}
	if b.escalate == nil {
		b.log.Error("worker: a fact went undispatched past its deadline and this backend has nothing to tell anyone with",
			"participant", string(snapshot.Participant), "worker", string(snapshot.Group),
			"kind", string(snapshot.Kind))
		return
	}
	stats := b.Stats()
	b.log.Warn("worker: a fact went undispatched past its deadline and is reaching the human",
		"participant", string(snapshot.Participant), "worker", string(snapshot.Group),
...
func (b *Backstop) Dispatched(ids ...ParticipantID) {
	...
	for key, cancel := range b.cancel {
		...
		if ws, ok := b.workerStore[worker]; ok {
			ws.owed--
			if ws.owed <= 0 {
				// The worker owes nothing, so the next fact starts fresh: it
				...
				// and workerOf would mint a fresh one that the next announce
				...
				// the moment a worker settles, not only when something goes
