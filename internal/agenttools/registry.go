...
var declarations = []Declaration{
	...
	{
		Name:        "workers.report",
		Description: "Tell the coordinator that started you, in your own words, that you have finished, that you need an answer, or that you have reached a milestone. Call it with `done` when the task is finished — or when you have stopped and it is not, which is what the text is for; nocx records no success and no failure and your coordinator judges what your words mean. Call it with `question` when you cannot continue without an answer: ask, then END YOUR TURN, because the call does not wait and the answer arrives as your coordinator's next message. Call it with `progress` at milestones, as often as you have them. This is your own worker session's call and nothing else's; you cannot report as anybody else or to anybody else, and neither is something you name.",
