...
# The worker's authority model: membership is not delegation
...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | **Membership makes a participant ADDRESSABLE; delegation makes it CONTROLLABLE.** They are two records, and neither implies the other                                                                                                                                                                    | One "participant" record carrying both. That is the conflation the bead is named for. Concretely: under it, adding a participant to a worker record would silently confer control over it, and revoking control would silently make it unreachable for the mesh talk the 2026-09-03 topology decision just granted                                                                                                                                                                  …
| A2  | **The delegation record is 2026-08-15 `D8`'s, adopted verbatim** — `Delegation{controllerSession, childSession, epoch, createdByRunId, effects, state}`, five effects, five states                                                                                                                       | A worker-specific authority record. A second vocabulary for one concept, which AGENTS.md's "look for the existing answer" forbids and which would drift from the workspaces design the first time either moved                                                                                                                                                                                                                                                                             |
...
| A11 | **A participant is addressed as a sub-scope of `ResourceWorkspace`**, whose canonical ids are already hierarchical (`workspace/<id>/tab/<id>/pane/<id>`)                                                                                                                                                 | A ninth `ResourceKind`. The kind set is closed at eight and guarded twice — `validResourceKind` (`internal/content/effectpolicy.go:130`) and the `grant_scopes` CHECK. `ResourceContent` already carries `note/<id>`, `snippet/<id>`, `skill/<id>`, so sub-scoping inside a kind is the established move. **`nocx-intbc` closed this** (`b29b1990`): `internal/agenttools/resourcekind.go` now measures the kind set against the ledger instead of restating it, so such a row assembles |
| A12 | **Until participant authenticity has a mechanism, a worker call carries NO authority the session does not already have**                                                                                                                                                                                   | Claiming the enrolment pin exists. It does not — see §5 — and a model that assumes "only the enrolled tree may speak for the participant" would be resting on a property no mechanism provides — `nocx-aqz7o` showed the candidate bearer was not one, and its fix was to DELETE the field                                                                                                                                                                                               |
...
| ------------------------------- | ------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| **What it proves**              | this participant is in this worker record                                                   | this controller may currently act on this participant          |
...
| **Held by**                     | the worker record                                                                       | a controller **session** (2026-08-15 `D8`)                     |
| **Revoked by**                  | leaving the worker record, or the worker ending                                             | A4's trigger set                                               |
| **On human takeover**           | unchanged — a taken-over worker is still in the worker record and still reachable          | `input-suspended`: `send-input` suspended, `observe` preserved |
...
**A taken-over worker does not vanish from the worker's holdings.** The coordinator keeps seeing it and keeps
...
| -------------------------------- | ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The per-epoch capability         | Implemented, and it is the ONLY binding today                             | **Corrected 2026-09-03.** This row read "it is written in cleartext on the outbound half of the descriptor every descendant inherits". `nocx-aqz7o` is now CLOSED (`182191e0`) and the fix was a DELETION: the bearer never authenticated a peer, because the shell's own hello hands that peer the capability a frame earlier. So the capability is not a weakened pin — it is not a pin at all, and the column head above is wrong: there is no binding today. The residual exposure is `nocx-zo2ng`, a nested grant still carrying the CHILD's capability on the parent's descriptor |
| `SO_PEERCRED` / `LOCAL_PEERCRED` | **Implemented on the tool endpoint (amended 2026-09-06, `nocx-rowqt.8`)** | Still true of the LIFECYCLE channel, which is a `socketpair`: no `connect(2)`, no per-peer credential to stamp, and loopback TCP has none by construction. It stopped being true of the SECOND caller's channel — `internal/toolendpoint` listens on a real unix socket, so the kernel stamps every peer and `D13`'s "OS identity from peer credentials" has its carrier there. Two channels, two answers                                                                                                                                                                               |
| `pidfd` / start-time pin         | **Built (amended 2026-09-06, `nocx-rowqt.8`)**                            | `internal/peerpin` pins the enrolled root as `(pid, startTime)` and walks a caller's ancestry to it, per OS: `/proc/<pid>/stat` parsed after the last `)` on Linux, `kern.proc.pid` on darwin. A pid reused after the root exits fails closed because the start time no longer matches, and a broken chain is refused rather than admitted. Still no `pidfd`; the start-time comparison is what makes the pid non-reusable                                                                                                                                                              |
...
**Therefore A12.** The worker ships with participant authenticity resting on exactly what the
session already rests on, and it says so rather than implying more. Concretely: a worker call may
...
**A12 survives the pin, amended 2026-09-06 (`nocx-rowqt.8`).** The pin now exists on the second
caller's socket, and it does not raise the ceiling. What it buys is separation WITHIN one uid: a
stale pid, a replacement process and an unrelated process that merely found the socket are told
apart from the tree that was enrolled. What it still does not buy is a defence AGAINST that uid —
a same-uid actor reads our descriptors and types into the session, so the principal remains the
TREE and never the person. And it is not approval: `D13` requires a human to admit an agent once,
seeing the executable and the scope, which is `nocx-rowqt.12` and is not built. Until it is,
nothing in the product may call the endpoint human-approved.
...
document — it is `nocx-zo2ng` and A12. **Causal ordering** and **loop prevention** remain open and
...
- **Whether reading output is separately granted from control in the general case.** A3 decides
  the worker's default bundle; 2026-08-15 §11 open question 3 asks it for every controller and
  stays open.

## 9. Assertions

1. A participant added to a worker record is addressable by every other participant and controllable by
...
3. A human takeover of a worker's pane leaves the coordinator's mail to that worker working, and
   leaves the worker record's membership intact (§4).
...
7. A `*WorkerParticipant` cannot be handed to a code path expecting a `*WorkerCoordinator`, proved by
...
10. No worker call permits an action the session's own grant would not (A12). Asserted by minting a
    participant capability from a fenced grant and finding it cannot exceed the fence.

## 10. What would falsify this design

- **A worker record in which membership confers control**, or in which revoking control removes
  reachability. Either collapses A1 and re-creates the defect the bead names.
- **A second authority record beside `Delegation`**, or a worker-specific effect lattice.
...
- **Any claim that the worker authenticates a participant**, while §5's table stands and
  §5's correction stands. That claim is the one thing here that would be dangerous rather than
  merely wrong.
