...
export interface HistoryRecord {
  ...
  entryId: string
  /**
   * The IMMEDIATE subject that submitted the command, in the ledger's own vocabulary (entries.source): 'user' is the human, 'assistant' is the assistant's lane. Minted at submit by the submitting InputTarget on the renderer and carried on the request (design §3.1, nocx-iadtt); the ack echoes the source the record was accepted under, so the renderer can verify the backend kept the fact it minted — the two sides never derive the same thing twice. A block whose source is not the human is visibly marked in the flow.
   */
