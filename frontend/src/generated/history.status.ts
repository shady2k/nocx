...
export interface HistoryStatus {
  ...
  detail: string | null
  /**
   * What happens to a session's output while NO client is attached. The backend's replay ring blocks its writer when it is full and nothing has consumed it (AD-10 — throttle the source, never drop); the acks that free it come from an attached client, so with nobody attached the recorder is the only consumer there is. Without one, a session whose window is closed keeps running only until the ring fills and then stops producing until somebody attaches. It rides beside `available` rather than as a reason for it: durable history may be running perfectly and this still be false, because it is governed by the output-retention switch and not by whether the store opened. This field carried a sibling, `discarded`, until nocx-lmb6v.4: the store used to REBUILD a file written by a different schema and say how many commands that cost. It migrates or refuses now (nocx-lmb6v.1), a refusal speaks through available=false with reason 'openFailed' and the refusal's own words in `detail`, and nothing discards a row at open, so the field was removed rather than left on the wire as a value no start can produce.
   */
  detachedOutput: {
    /**
