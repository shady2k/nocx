...
export interface AgentRunToolCall {
  ...
  opensBlock: boolean
  /**
   * The LEDGER action entry the attempt was recorded under (design §6.4). The thread joining question, run, attempt and answer — and the handle a later 'show me what it returned' reaches through, rather than a second copy of the bytes. That reach is real: the call's result is this entry's body artifact (nocx-hp8p2.13), fetched one call at a time by whoever opens one, so a turn of ten calls costs ten bodies only if a person asks for ten.
   */
