...
func TestMCPOAuthPresenterRefusesNonHTTPURL(t *testing.T) {
	opener := &fakeUrlOpener{}
	err := (mcpURLPresenter{opener: opener}).PresentURL(t.Context(), "file:///tmp/oauth")
	if err == nil {
		t.Fatal("file OAuth URL was accepted")
	}
	if opened := opener.opened(); len(opened) != 0 {
		t.Fatalf("file OAuth URL reached the platform opener: %v", opened)
	}
}
