...
func TestFloor_RefusesUnderMostPermissivePolicy(t *testing.T) {
	...
	k, err := newEffectKernel(nil, grant, reg, &fakeLedger{}, nil, &fakeKnownMaterial{}, "run-floor", "session-floor", 1, "", nil, nil, nil)
	if err != nil {
...
func TestNewClient_InjectsFloorBeforeToolExecution(t *testing.T) {
	...
	client, err := NewClientWithoutSkillRoots(nil, nil, content.NewFloor(configDir, filepath.Join(t.TempDir(), "data", "nocx")))
	if err != nil {
...
func TestFloor_ReasonIsTruthfulForReadAndWrite(t *testing.T) {
	...
	k, err := newEffectKernel(nil, grant, reg, &fakeLedger{}, nil, &fakeKnownMaterial{}, "run-floor-reason", "session-floor-reason", 1, "", nil, nil, nil)
	if err != nil {
		t.Fatalf("newEffectKernel: %v", err)
	}
