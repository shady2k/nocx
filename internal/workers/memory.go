...
// EnsureGroup records a worker and does NOTHING if it is already there. It never
// reassigns: the coordinator session of a worker is its identity, and a second
// call moving it would hand every participant to a controller that never
// spawned them.
func (s *MemoryStore) EnsureGroup(_ context.Context, id ID, coordinatorSession string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, ok := s.workerStore[id]; ok {
		return nil
	}
	s.workerStore[id] = coordinatorSession
	return nil
}
...
func (s *MemoryStore) MarkLive(_ context.Context, id ParticipantID, l Liveness) error {
	...
	if !ok || p.State != StatePrepared {
		return fmt.Errorf("worker: mark live %q: %w", id, ErrNoSuchParticipant)
	}
	p.State = StateLive
	p.Liveness = l
	s.parts[id] = p
...
func (s *MemoryStore) Terminalize(_ context.Context, id ParticipantID, st State) error {
	if !st.Terminal() {
		return fmt.Errorf("worker: terminalize %q: %q is not a terminal state", id, st)
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	p, ok := s.parts[id]
	if !ok {
		return fmt.Errorf("worker: terminalize %q: %w", id, ErrNoSuchParticipant)
	}
	if p.State.Terminal() {
		return nil
	}
...
func (s *MemoryStore) RecordExit(_ context.Context, id ParticipantID, e Exit) (Participant, error) {
	...
	p, ok := s.parts[id]
	if !ok {
		return Participant{}, fmt.Errorf("worker: record exit %q: %w", id, ErrNoSuchParticipant)
	}
	stored := e
	p.Exited = &stored
	s.parts[id] = p
...
func (s *MemoryStore) Delegation(_ context.Context, id ParticipantID) (Delegation, error) {
	...
	d, ok := s.dels[id]
	if !ok {
		return Delegation{}, fmt.Errorf("worker: no delegation over participant %q: %w", id, ErrNotDelegated)
	}
...
func (s *MemoryStore) Participant(_ context.Context, id ParticipantID) (Participant, error) {
	...
	p, ok := s.parts[id]
	if !ok {
		return Participant{}, fmt.Errorf("worker: participant %q: %w", id, ErrNoSuchParticipant)
	}
	return copyParticipant(p), nil
}

// CoordinatorSession answers who must judge a fact about this workers. A worker
...
func (s *MemoryStore) CoordinatorSession(_ context.Context, id ID) (string, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	coord, ok := s.workerStore[id]
	if !ok {
		return "", fmt.Errorf("worker: worker %q: %w", id, ErrNoSuchParticipant)
	}
	return coord, nil
}
...
func (s *MemoryStore) ParticipantBySession(_ context.Context, sessionID string) (Participant, error) {
	if sessionID == "" {
		return Participant{}, fmt.Errorf("worker: session %q: %w", sessionID, ErrNoSuchParticipant)
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	return Participant{}, fmt.Errorf("worker: session %q holds only terminal participants: %w",
...
// worker disappear from its own coordinator's account of the workers.
//
...
func (s *MemoryStore) HeldBy(_ context.Context, coordinatorSession string) ([]Participant, error) {
	...
	defer s.mu.Unlock()
	return s.selectParticipants(func(p Participant) bool {
		return s.workerStore[p.Group] == coordinatorSession && p.Role != RoleCoordinator
	}), nil
}
...
func (s *MemoryStore) Commit(_ context.Context, m Message) (Message, error) {
	id, err := uuid.NewV7()
	if err != nil {
		return Message{}, fmt.Errorf("worker: mint a message id: %w", err)
	}
...
// Undelivered lists what a worker's mailboxes hold that their OWN RECIPIENTS
// have not fetched.
//
