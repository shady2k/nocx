...
3. .internal/plans/2026-09-01-assistant-skills.md — read the header and the
   "Global Constraints" section (lines 1-30), then "### Task 6" in full
   (lines 979-1147). That is your task; it contains the code for every step.

Your bead id is nocx-tbxwq. Every commit subject must end with "(nocx-tbxwq)".
...
- Do not widen scope beyond Task 6. If you find a defect outside it, put it in
...
Task 4 (already merged) landed `profile.RoleSummarizing`, and in
`internal/assistant/skilldraft.go` the two functions `ComposeDraftInput` and
`DraftSkill`. They have **no production caller**, because the consumer the plan
named for them is exactly the thing you are building: the kernel resolving the
summarizing role, calling the draft, and handing the result to the person's
approval as the proposed `skills.create` arguments. Task 4 was sequenced before
the tool it consumes.

That wiring is its own bead — nocx-9ysnr — and it is NOT your task. Do not do it.
What is asked of you is only this: build `skills.create`, `skills.update` and
`skills.delete` so that the arguments a draft produces (`name`, `description`,
`body`) are exactly the arguments `skills.create` accepts, and say plainly in your
final report whether they line up with `assistant.DraftSkill`'s return values. If
they do not, that is a finding worth more than a workaround — report it, do not
bend `skills.create`'s shape to a draft function you were not asked to wire.
