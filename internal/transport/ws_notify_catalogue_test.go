...
func TestNotifyCatalogue_OverTheWireConformsToContract(t *testing.T) {
	...
	want := []catalogueKind{
		...
		{ID: "coordinatorStalled", Kind: "coordinator.stalled", Label: "A coordinator is not reading its workers", Description: "nocx wrote to a coordinator about its workers' mail and the coordinator did not read it, or the coordinator is blocked on its own screen while its workers are live."},
