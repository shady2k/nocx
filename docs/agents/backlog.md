...
0.26.0 on 2026-09-25 by its setup task, "The backlog tooling here matches shady2k-skills
0.26.0 and is proved from main" (nocx-q8yjf.9). The protocol itself ships with the skills
...
- **Rules:** [`check.mjs`](../../.githooks/backlog-gate/check.mjs),
  ...
  of the shady2k-skills plugin's `skills/backlog/setup-shady2k-skills/` at 0.26.0, never
  edited here. Proving it: `cmp` each against the plugin copy, `--version` prints `0.26.0`,
...
- **Connecting a clone:** `make connect` (`scripts/connect-clone.sh`), which `make init`
  ...
  hooks, the gate's `adapter.mjs`, `check.mjs`, `check-commits.mjs`, `commit-links.mjs`
