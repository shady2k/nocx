-- SCHEMA 17, VERBATIM. This is `schemaV1` as it stood on main at 95a40db3,
-- the last commit holding `const schemaVersion = 17` before the
-- termination_reason widening moved it to 18 (nocx-4yjwk.7) — lifted
-- unmodified so a test can fabricate a database a real build wrote rather
-- than one a test invented.
--
-- 17 is schema 16 plus `skill_checks` and nothing else (the 16→17 rung is
-- purely additive), and it is where `executions.termination_reason` still
-- admitted nine values and not `answer-revoked`. The 17→18 rung widens that
-- CHECK, and the rows a test writes into this file are what the rebuild has
-- to carry across.
--
-- IT IS FROZEN, for the reason schema_v14.sql states at length and
-- schema_v16.sql repeats: a fixture edited to keep up with the current schema
-- stops being a fixture, because the migration it feeds would then be tested
-- against the shape it produces rather than against the shape it has to carry.
-- When 18 is no longer the version above this one, this file stays exactly as
-- it is and a schema_v18.sql joins it.
