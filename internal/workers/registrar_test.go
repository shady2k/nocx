...
func (m *memStore) EnsureGroup(_ context.Context, id ID, coord string) error {
	if err := m.hit("ensureworker"); err != nil {
		return err
	}
	m.mu.Lock()
	defer m.mu.Unlock()
	m.workerStore[id] = coord
	return nil
}
...
func (m *memStore) nonTerminal(_ context.Context, id ID) ([]Participant, error) {
	...
	for _, p := range m.parts {
		if p.Group == id && !p.State.Terminal() {
			out = append(out, p)
		}
	}
	return out, nil
...
func (m *memStore) Undelivered(_ context.Context, id ID) ([]Message, error) {
	...
	for mailbox, msgs := range m.mail {
		...
		for _, msg := range msgs {
			if msg.Group == id && msg.Seq > fetched {
				out = append(out, msg)
			}
		}
	}
...
func (m *memStore) CoordinatorSession(_ context.Context, id ID) (string, error) {
	...
	m.mu.Lock()
	defer m.mu.Unlock()
	coord, ok := m.workerStore[id]
	if !ok {
		return "", fmt.Errorf("no such worker %q: %w", id, ErrNoSuchParticipant)
	}
	return coord, nil
}
...
func (m *memStore) HeldBy(_ context.Context, coord string) ([]Participant, error) {
	...
	var out []Participant
	for _, p := range m.parts {
		if m.workerStore[p.Group] == coord {
			out = append(out, p)
		}
	}
	m.mu.Unlock()
