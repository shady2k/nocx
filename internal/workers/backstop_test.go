...
// a worker with nothing to judge has no timer at all. A lease would be ticking
// here, through a worker that is entirely healthy and silent.
func TestAGroupWithNothingUndispatchedWakesNobodyAndRunsNoTimer(t *testing.T) {
...
// coordinator fetches it, so an idle worker returns to no timer running rather
...
func TestAFactThatNeedsJudgementWakesTheCoordinator(t *testing.T) {
	...
	if !strings.Contains(calls[0].text, "workers.holdings") {
...
func TestADeclarationEntersTheSetAndWakesTheCoordinator(t *testing.T) {
	...
	// The coordinator is read from the WORKER, so a wake reaches the session
