...
- **Config / data locations.** `profiles.json` and `settings.json` via an atomic `DocumentStore` (temp+rename, `0600`), stored in the OS config dir. The OS keychain stores credentials and secret-class settings separately from documents. A future `content.db` (SQLite, WAL mode) will store conversations and command history in the application-data dir. A versioned plaintext backup projection (`nocx-backup` v1, ADR-0018) exports non-secret settings, connections, and groups without credential records or keychain material. A local transaction journal (`backup-restore-journal.json`) guards crash-safe restore, and a poisoned configuration gate blocks all config mutations after an unrecoverable journal state until restart.
...
- **Alternative SecretStore** — the OS keychain is the primary secret store per ADR-0011. If an OS keychain proves unavailable or unusable on a supported platform, add an app-managed encrypted secret store as a second `SecretStore` implementation behind the existing interface (ADR-0011 revisit condition).
