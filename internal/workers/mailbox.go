...
var (
	// ErrNotAMember means the sender or the recipient is not in the worker.
	...
	ErrNotAMember = errors.New("worker: not a member of that worker")
	// ErrMessageTooLarge means the body exceeds MaxMessageBytes.
	ErrMessageTooLarge = errors.New("worker: message is too large")
	...
	ErrEmptyMessage = errors.New("worker: message is empty")
)
