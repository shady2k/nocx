...
func TestHelperABIVersionRejectsKernelsBelowTheEnforcementFloor(t *testing.T) {
	original := landlockABIQuery
	t.Cleanup(func() { landlockABIQuery = original })

	landlockABIQuery = func() (int, error) { return minLandlockABI - 1, nil }
	if got := landlockABIVersion(); got != 0 {
		t.Fatalf("helper ABI = %d, want 0 below floor %d", got, minLandlockABI)
	}
}
