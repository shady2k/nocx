-- SCHEMA 15, VERBATIM. This is `schemaV1` as it stood in the tree at
-- `const schemaVersion = 15`, lifted unmodified out of 6035c0cf~1 — the last
-- commit before the api-run counter was folded into the ladder and the stamp
-- became 16.
--
-- IT IS FROZEN, for the reason testdata/schema_v14.sql states at length.
--
-- It arrived late (nocx-4yjwk.7). `schemaShapeDigests[15]` had been a constant
-- with no database behind it: nothing in the suite ever built a file stamped
-- 15, so the pin could not be checked and a wrong one would have refused every
-- genuine schema 15 database on sight. This file is what makes that pin
-- derivable and testable, like 14's and 16's.
--
-- A released 15 file also carries the api-run tables and their private
-- counter, which schemaV1 did not own until 16; aDatabaseAtSchema15 adds them
-- from testdata/api_runs_two_counter_era.sql exactly as the 14 fixture does.
