...
- Rule: one WebSocket per client, split into two planes; sessions multiplexed by server-assigned session-id. Wire format is explicit:
  ...
  - **Presentation requests may cross too** (amended 2026-08-14, ADR-0047, nocx-uz7f). The enumeration above is extended from facts about a completed command to **presentation requests**: a parsed, expressly registered terminal sequence by which a program asks nocx to present a message (OSC 9, OSC 777). The test above is unchanged and still governs. What this is not: a program may ask, and it never chooses where the message goes — ADR-0047 binds every such record with the destination, provenance, trust-class, secret-bearing-URL and retention rules, and no program-initiated effect may cross without them.
  ...
  - **The data plane stops carrying raw PTY bytes, in both directions** (amended 2026-09-12, [ADR-0066](decisions/0066-one-emulator-and-it-is-the-backends.md), nocx-g5p8c). The session runtime owns the emulator (AD-6, amended the same day), so outbound the data plane carries **frames** — the cells that changed, the cursor, the committed geometry — and inbound it carries **structured input**: a key event, text, a paste, a mouse event, a focus change. A frame of cells conveys nothing about `DECCKM` or bracketed paste, so a client that encoded its own keys would send differently encoded input while the screen looked correct; encoding moves to where the modes live. **The two-plane split is untouched** — the data plane is still binary, still `version || msg-type || session-id || payload`, still never JSON, and the control plane is still JSON-RPC 2.0. What changed is what the payload MEANS, not the shape of the socket. The delivery promise over those frames is AD-10's, and it changed too.
...
- Rule: SSH sits behind a clean interface; validate a destination before any `ssh -G` or connection-resolution subprocess can parse it; honor `~/.ssh/config` via a config parser (e.g. `kevinburke/ssh_config`); SFTP via `pkg/sftp` later; the vault injects credentials through this library. The `ssh` module owns a **ref-counted `ssh.Client` connection pool** keyed by host+identity: channels multiplex over one connection, and the connection closes with the last tab that references it — preserving connection reuse and Phase-2 vault credential caching.
...
- Rule: the VT frontend (xterm.js — [ADR-0001](decisions/0001-xterm-js-as-vt-frontend.md)) owns render state **for the live region** — the grid it is currently drawing, and selection within it — and parses OSC 7/133, surfacing them as events via `parser.registerOscHandler` (verified, `nocx-dej`); the Go backend owns PTY/session lifecycle, SSH connections, and config/vault. The backend does **not** sniff the byte stream.
- Amendment, [ADR-0009](decisions/0009-dom-scrollback-with-explicit-cell-geometry.md): **the SCROLLBACK is not xterm's.** When a command ends its rows are serialized into a frozen block and leave the grid, and from that moment the DOM owns their presentation and their selection. This has been true since 2026-07-24 and this line said otherwise until 2026-09-05 — long enough for a design review to reason from it and reach the wrong product. Single ownership still holds; the owner changes at the freeze. Which layout the frozen half uses is ADR-0009's subject, not this invariant's.
  ...
  - **A live grid for an enrolled pane, and it does read the stream's meaning** (amended 2026-08-25, `nocx-szb40.2`, measured in [ADR-0041](decisions/0041-x-vt-as-the-backend-emulator.md)). **This one crosses the rule above; the bullet before it does not, and the difference is the point.** That carve-out reads a framing _we_ wrote and interprets nothing. This one maintains a full VT grid in the backend and derives meaning from what an application drew — precisely what this AD forbids and what [ADR-0002](decisions/0002-native-tabs-no-embedded-multiplexer.md) closed twice. It is permitted because ADR-0002's own revisit trigger has fired: a session that survives the client process entirely is now a product requirement (`nocx-22k1c`, `nocx-eidfb`, `nocx-dkawo`). The containment is not that the backend interprets less. It is the **interval** and the **two powers**, and nothing else is doing any work.

    **The interval, and what opens it.** A grid exists for a pane that has been **explicitly enrolled for observation** by a control-plane call naming that pane, and for no other pane. Enrolment is an act, never an inference: nocx does not decide a pane deserves a grid because its title looked like an agent, because that has no upper bound and no audit. A participant in a wave is one producer of such an enrolment and, at the time of writing, not the only one — a person starting an agent in a pane is another. The interval opens at enrolment, over a process nocx **launched**, so the grid starts at byte zero and never infers state accumulated before it was watching; it closes when that pane's record becomes **durably terminal** or the enrolment is withdrawn, at which point the grid is discarded and the pane is never read again. A pane nocx **adopted** rather than launched has no byte-zero guarantee: its state is `unknown` until a full repaint has been observed, and `unknown` refuses every power below. An unenrolled pane has no backend grid at any point, and its indicator stays where it is today (`frontend/src/agent-status.ts`).

    **The act, named (amended 2026-08-25, `nocx-szb40.5`).** The control-plane call is the `agent_enrol` / `agent_withdraw` pair on the authenticated channel of [ADR-0024](decisions/0024-authenticated-shell-integration-channel.md) decision 2 — [the protocol §15](lifecycle-protocol.md#15-agent-enrolment-the-interval-a-backend-grid-lives-in) — sent by an agent wrapper in the shell-integration bundle, which brackets the agent it runs. It is on that channel and not a socket of its own because a second socket would be a second authenticator for one trust decision, and because the per-epoch capability lives in the integration script's text and never in the environment, so nothing a shell merely spawns can authenticate at all. The interval therefore has two ends that do not depend on each other: the wrapper's withdrawal when the agent returns, and the backend's own withdrawal when the session's output ends, which covers a wrapper that was killed rather than returning. **The grid follows the pane's geometry** for the life of the interval, from the pty's own resize path — both powers below are positional, so a grid left at the size it was enrolled at does not answer about a stale screen, it answers about a screen that never existed.

    **The two powers, exhaustively.** Inside that interval the grid MAY decide exactly two things: **(1)** whether nocx may write into this pane — the refusal in `nocx-dkawo.1`, where anything but a positively identified `free_text` receives nothing at all; and **(2)** what the pane's activity indicator shows. That is the whole list, and it does not grow with the number of enrolled panes.

    **What it may never decide, and this is not a general licence.** The grid may not open, complete, alter or assign status to a **wave state**, a **lifecycle attempt**, or an **execution attempt**. That authority is the authenticated channel's ([ADR-0024](decisions/0024-authenticated-shell-integration-channel.md) decision 2) and process exit's, and stays there — a fact a participant _declared_ and a fact the PTY _reported_ are the only things that move a wave. The grid may not reach a network destination: an inference from stream content is `internal/notify`'s `heuristic` trust class, which the routing table refuses to give a sink that leaves the machine. The renderer remains the single owner of the user's VT state, scrollback and OSC handling; this grid is a second, private reading for observation only, and nothing derived from it is shown to the user as their terminal or persisted as their history. The backend still parses no OSC and no VT **for any pane that is not enrolled**, and enrolment is bounded by an explicit call rather than by a guess about what a pane is running.

    **What makes it safe is the window and the two powers, never the parser.** Being wrong here costs a refused keystroke or a mislit dot — both recoverable, both visible. It cannot cost a false completion, because completion is not one of the two. If a future reader wants a third power, that is an amendment to this bullet and not an extension of it.

    **The first power has a second case, and the reading may reach the session that holds the pane** (amended 2026-09-11, `nocx-f545a.1`, [ADR-0064](decisions/0064-a-pane-that-is-read-may-be-answered.md)). The powers are still **two**; one of them now has two cases. nocx may write into an enrolled pane when the pane is a positively identified `free_text`, and what is written is then arbitrary text — unchanged; **or** when the pane is a **positively identified menu**, and what is written is drawn from the **closed set of keys that menu offers**: move the selection, confirm it, dismiss it, and nothing else. The selection is named by **the option's own text as the screen drew it** — never a key, a row number or a coordinate — because nocx turning a name into a key is what keeps the set closed, and the frame is re-read immediately before the write, so the identification that authorises it is the one taken microseconds before rather than the one a caller saw. Beside that write, two READS may cross to a session holding a live delegation over that participant, and to no other caller about no other pane: what the pane is, with a menu's question and options as drawn; and the pane's screen as rows of text, through the SAME projection `agent.emitting` already renders and persisted nowhere. **A person's own pane is not readable this way by anybody** — the delegation points one way and this permission points with it. What is still refused is the whole of the paragraph above, restated because an addition is where a reader looks for the limits: a menu answer moves **no** wave state, lifecycle attempt or execution attempt; nocx **never** chooses an option itself, not on a timeout and not to unblock a spawn; and a pane's own agent may not answer its own menu. The safety argument is re-made rather than inherited, because a keystroke into a misread dialog is not a refused keystroke: it rests on the identification, not on the consequence — nocx sends a key only where it has positively identified a menu **and** the key is one that menu itself offers, so being wrong means the menu was misidentified rather than that a keystroke went somewhere unbounded. ADR-0064 also records what this costs and why it was taken anyway: a coordinator is a model, so a screen handed to it leaves the machine, which is why the boundary is the delegation and not "any enrolled pane".
  ...
  - **The backend owns the emulator, and the RULE above no longer holds** (amended 2026-09-12, [ADR-0066](decisions/0066-one-emulator-and-it-is-the-backends.md), nocx-g5p8c, design §4). Read this bullet before any of the ones above it: the two carve-outs preceding it were exceptions to "the backend does not sniff the byte stream", and that sentence is now superseded. One VT emulator exists per session, in a long-lived runtime beside the PTY, and what it derives IS the user's terminal rather than a private second reading. The refusal list forbidding grid-derived content from being displayed or persisted goes with it. Single ownership is not weakened — it MOVES: the runtime owns the screen, the terminal's modes, the answers to the program's own questions, the committed geometry and the order input is admitted in; the renderer owns layout, painting, selection, accessibility and IME, and what a person does arrives as intent rather than as bytes. xterm.js is removed, not demoted (ADR-0001, superseded).

    **The grid's LIFETIME is superseded too, and it is a separate thing from its powers.** The 2026-08-25 bullet binds when a grid may exist at all — only for an explicitly enrolled pane, discarded at withdrawal, and "an unenrolled pane has no backend grid at any point". An emulator per session contradicts all three sentences, so all three go. Enrolment stops being what CREATES terminal state and becomes an observation authority over a runtime that already exists (`internal/app/paneenrol.go:102` creates the grid and refuses an existing one today; that inverts).

    **What stands verbatim is the list of what a grid may not DECIDE**: no wave state, no lifecycle attempt, no execution attempt, no network destination. Those are POWERS and they are not consequences of where the parser runs. It is written out again here because "a grid may now exist everywhere" reads as "a grid may now decide more" unless it is refused out loud. It decides no more than it did. ADR-0064's second case stands; its re-read-before-the-write mechanism is superseded below, by ADR-0067's commit-point validation. The bootstrap-window carve-out above is untouched either way: parsing VT is not permission to interpret bootstrap readiness tokens or to write bootstrap frames outside their interval.

    **A third power: telling a coordinator what its worker's pane shows** (amended 2026-09-17, [ADR-0070](decisions/0070-a-worker-says-nocx-sees-the-coordinator-judges.md), `nocx-luqz9`). The classification of a worker's pane — idle, blocked — may be delivered to the mailbox of the coordinator holding that worker, as an observation carrying no screen content. It moves no record state: a worker's record is moved by spawn, process exit and close only, and nocx records no outcome at all. The refusal list above stands.

  - **The first power is conditional input under a helper-minted, one-shot target, not a closed key set** (amended 2026-09-14, `nocx-6q1uh.12`, [ADR-0067](decisions/0067-a-write-into-a-descendants-pane-is-one-step-under-a-helper-minted-target.md)). The powers are still **two**. Power (1) is restated again: nocx writes into a descendant's pane one state-changing step at a time — one key, one text atom written as a paste, or one menu option named by its own text — under a **target** the helper mints from the exact snapshot the caller classified, carrying a structural digest of that snapshot's rows, cursor and geometry, and consumed exactly once at the write. The keys a positively identified menu accepts are no longer closed by enumerating what it offers: a target that does not match the frame at the write is refused (`incomparable`) regardless of which key was sent, which is a narrower guarantee than the closed list it replaces, not a looser one. **The re-read immediately before the write moves from the caller to the session I/O owner's commit point** — the party that now holds the frame and the PTY together, since ADR-0066 put the one emulator in the helper rather than beside the coordinator's own agent-typing path — which validates the token there instead of the caller re-reading a microsecond before sending. This power now reaches every pane a caller's descendant chain resolves to through its delegations, not only a directly held participant (ADR-0067, extending ADR-0064 §2); a pane that is not a descendant, including a plain shell, is refused `not_reachable`, never answered. What is still refused stands exactly as above: no wave state, lifecycle attempt or execution attempt moves this way, nocx never chooses an option itself, and a pane's own agent may not answer its own menu.
...
- Rule: the backend holds a **bounded per-session output ring** keyed by a monotonic byte-offset; the frontend acks the last-received offset, and only the connection currently registered as that session's subscriber may advance the ring. On reconnect the frontend sends its last offset and the backend replays from there, or emits an explicit `reset` (clear + resync) if the offset is past the buffer.
  - This replay ring is **transport buffering, not scrollback ownership** — scrollback stays frontend-owned, so AD-6 is intact.
- **Amendment (2026-09-12, [ADR-0066](decisions/0066-one-emulator-and-it-is-the-backends.md), nocx-g5p8c): replay is not how a client reaches the same screen.** The rule above recovers a client by handing it bytes it must EXECUTE, which is why it needed a reset when the offset fell past the buffer — and a reset is a cleared screen. With the emulator on the backend, an attaching or reconnecting client receives **a snapshot revision** — the cards through revision R and the live frame at R — and then the changes after R. Geometry, the active presentation, the running block and the availability of an artifact all belong to that same revision, because a card sealed between two independent reads is otherwise in neither of them. The `session.output` contract and its no-grid reasoning are superseded with the rule. **What the ring is still for**: transport buffering and AD-10's one lossy case on a helper-owned host, unchanged.
...
- **Amendment (2026-09-26, [ADR-0075](decisions/0075-an-overflowing-row-stream-ends-the-block-in-flight-incomplete.md), nocx-2v80t.3.40): the block row stream does not record through its hole.** The sentence above, "the recording records the hole rather than stopping at it", still binds the helper's output window, but no longer the stream of rows into block history. When either side's row buffer overflows, the block in flight ends **incomplete** and recording resumes at the next command's end marker, so an end is never attributed to a block it does not name. Both buffers are the person's settings (`history.helperBufferMB`, `history.coordinatorBufferMB`). The helper clamps its buffer to a floor, a ceiling and the same helper-wide aggregate budget as the window; the coordinator's bound counts every queue that holds rows or closing screens.
- **Amendment (2026-09-12, [ADR-0066](decisions/0066-one-emulator-and-it-is-the-backends.md), nocx-g5p8c): frames are not bytes, so "lossless and ordered" no longer describes the whole promise.** Three classes, and every payload belongs to exactly one. **Lossless and ordered** still binds what must never be lost: the ingest into the emulator, and the ledger. **Coalescable** is what frames are — intermediate visual states nobody was shown are explicitly lossy, because that is what frame-rate coalescing IS, and calling it lossless would be a promise the design cannot keep. **At-most-once with a duplicate policy** is for effects that change no cell: a bell, a notification request, a clipboard write, a title. A full frame must never repeat one. Bounded per-session credit and per-session fairness still bind, expressed over frames rather than over bytes. **This invariant is therefore superseded in its shape before its replacement is written, and that is stated here rather than left to be discovered** — `nocx-ygxjv.4` owes the contract, in `contracts/` where it crosses the wire.
- **Amendment (2026-09-12, nocx-ygxjv.4) — the replacement exists, and it is `internal/sessionruntime`'s.** The three classes are declared there as vocabulary: `DeliveryClass` (`DeliveryLossless`, `DeliveryCoalescable`, `DeliveryAtMostOnce`), `EffectKind` and `Effect`, the latter carrying the `EffectID` that makes a duplicate recognisable — two deliveries of one identity are one bell — and the bounds as NUMBERS rather than adjectives: `MaxIngestBytes` (one ingest call; a larger one is REFUSED rather than truncated), `MaxPendingSequence` (the bytes of an unterminated escape sequence the runtime will hold; the excess is dropped and the loss is stated, never a silent trim) and `MaxPendingFrames` (one session's consumer queue; the excess is coalesced and reported, and the allowance is per session and never transferable). Two of the three classes are now asserted by schedules, each paired with the removal of the one rule it rests on: a consumer that NEVER READS cannot grow the runtime past `MaxPendingFrames`, cannot cost the lossless ingest a byte, cannot spend another session's allowance, and is TOLD what it lost; a hostile capture costs work linear in the bytes the runtime examined and never in a count inside them, and an unterminated OSC or a DCS larger than any bound is bounded or refused. **Which bound is the EMULATOR's is named in the contract rather than implied** — `REP`'s clamp (libghostty-vt's 65535 against x/vt's projected ~180 s and ~10⁹ allocations, ADR-0065) and a real parser's own limits are `nocx-ygxjv.2`'s, and no schedule there measures an emulator that does not exist yet. **No `contracts/` schema accompanies it, deliberately**: these classes are the runtime's own vocabulary, and the shape they cross the wire in IS the frame format, which is epic `nocx-zg3k3`'s. A schema written now would describe a shape nothing sends, which is the failure `…_OverTheWireConformsToContract` exists to catch, so the wire half follows the frame format rather than preceding it.
