...
| `mcp`              | Own dormant-on-start MCP stdio/Streamable HTTP runtime, bounded process/network lifecycle, OAuth activation, and untrusted catalog/result translation. It has no policy authority; assistant execution remains in `effectKernel`. |

MCP tools are a local declarative catalog, not a provider-native extension. Settings `Refresh tools` is the only discovery trigger. Enabled fresh catalog rows are composed into an immutable per-run `agenttools.Registry`; every invocation passes the existing schema → policy/floor → destination scope → approval → ledger → narrowed capability pipeline before `internal/mcp` activates a server. MCP effects are always `delegate`, and destinations are canonicalized separately from availability. Secret values remain behind the Vault type boundary; profile and wire records contain opaque references only. Runtime sessions are keyed by `(runID, serverID)` and close on terminalization, cancellation, suspension, mutation, stale descriptors, idle timeout, and shutdown.
...
- **Config / data locations.** Plain files in the OS config dir — `~/Library/Application Support/nocx` on macOS. Settings/themes/keybindings/tab-restore and the profile aggregate (including MCP server declarations and sanitized catalogs) persist locally. MCP records contain opaque secret references only; the current backup format deliberately excludes endpoint-like MCP records. The Vault remains a separate encrypted, single-machine store with no sync.

**MCP security envelope.** `internal/mcp` accepts immutable activation snapshots, never Settings or unrestricted Vault capabilities. Static bearer/custom headers are assembled from secret references at activation; OAuth browser authorization is available only from the explicit Settings action. Endpoints are HTTPS or explicitly configured loopback/private HTTP, redirects are bounded and origin-checked, and process/network bodies are bounded. MCP descriptions, annotations, stderr, and results are untrusted data and pass existing egress/result bounds. No MCP activation occurs before effect-kernel approval and ledger admission; `delegate` is the only MCP effect.
