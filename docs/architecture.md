...
- Rule: one WebSocket per client, split into two planes; sessions multiplexed by server-assigned session-id. Wire format is explicit:
  ...
  - **Bulk file bytes travel beside the two planes, not on them** (amended 2026-08-21 by [ADR-0036](decisions/0036-an-http-upload-route-beside-the-websocket.md), and 2026-08-28 by [ADR-0040](decisions/0037-an-http-download-route-beside-the-websocket.md)). Browser transfers use streamed, single-use-ticket routes on the loopback listener: `POST /upload/{ticket}` toward the tab's host and `GET /download/{ticket}` back to the browser. A Wails desktop download claims that same authorised transfer through `files.downloadSave` and streams the pinned source into an injected local `transfer.Sink`; no file byte or destination path crosses the WebSocket or renderer. The control plane still owns minting, binding/session authorisation, cancellation, progress and completion. These are closed exceptions for bulk file payloads: PTY bytes remain on the binary data plane, control messages remain JSON-RPC, no data-plane msg-type is consumed, and another route or payload class requires its own ADR.
