...
| **ADR-0019** — one authoritative ledger, disposable projections                                                                                                       | The ledger is the record of what happened                                                                                                                                                                                                                                                                  | Unchanged. The ledger is **not** the sink for detached output: `internal/transport/ws_history_record.go:187` says the output capture path "does not exist yet", and building it would put terminal bytes in a second owner beside the renderer's scrollback.                     |
| **ADR-0024 / `0035-the-channel-we-own-is-the-carrier.md`** — shell integration delivered over the channel we own                                                      | Integration travels on the channel, not in the exec command                                                                                                                                                                                                                                                | Phase B changes the carrier, not the mechanism: the thin binary spawns the shell itself, so integration no longer needs rc-file editing or SFTP delivery. Markers still come from the shell — AD-6 is untouched.                                                                 |
...
- **D6 — With no client attached, the ring's credit is extended by a bounded per-session
  disk spill — and AD-10 must be amended either way.** The ring is 256 KiB (`ring.go:14`)
  and blocks its writer when full (`ring.go:74`, implemented at `:83`); the acks come from
  a client. With nobody attached nothing acks, so a detached session freezes after 256 KiB
  — the daemon would stall the very work it exists to protect.

  **This is the design's one open decision, and it belongs to the owner, because AD-10 as
  written admits no third answer.** AD-10 (`architecture.md:188`): bounded credit per
  session; when the credit is exhausted, throttle the source; _never drop, never grow
  unbounded_; **bytes are lossless and ordered**. A bounded spill that discards its oldest
  content is not lossless. So:

  - **D6-a — lossless, and the work stops at the bound.** The spill extends the credit; at
    the bound the source throttles, exactly as AD-10 says. A detached session survives up
    to N bytes of output and then waits for you. AD-10 is amended only to say the credit
    may be backed by disk when no client is attached. Nothing in nocx ever loses a byte.
  - **D6-b — the work always continues, and the middle of the log may be lost.** At the
    bound the spill discards its oldest bytes and the reclaim reports an explicit gap.
    AD-10 is amended to permit exactly one lossy case — a session with no attached client
    — and to require the loss be stated in the product.

  What is **not** a precedent for D6-b: today's `reset` fires when a client asks for an
  offset it had already acked and the ring has since trimmed. Unacked bytes are never
  discarded. The system is lossless today, and D6-b would be the first place it is not.

- **D7 — The reclaim outcome must be visible, and the renderer does not do this yet.** The
  review found the claim that it already does to be false: a reset returns `'resumed'`
  (`frontend/src/ipc.ts:419`), and the `{resumed, lost}` aggregate (`:444`) counts sessions
  the backend no longer holds — it does not report a replay gap. The hook exists
  (`state.resetCallback`, `:417`); the notice does not use it.

  D7 also owes a definition AD-9 promises and the code does not yet provide: `reset` today
  clears the decoder and the renderer (`frontend/src/terminal-content.ts:3018`) while the
  backend returns no state when the requested offset is gone (`ring.go:157`). "Clear +
  resync" must say what the client is resynchronised _to_, and replay must not begin inside
  a UTF-8 sequence or a VT escape.

- **D7a — the spill cursor is not an acknowledgement.** AD-9 (`architecture.md:181`) and
  `ring.go:186` both mean by "ack" that the **frontend** received the bytes. A cursor saying
  the coordinator persisted them frees the memory ring, but it is a different fact with a
  different invariant. Two cursors, two names — conflating them is how one silently acquires
...
- Then a front of three, each depending only on A1.0:
  - **A1.1 — session continuity.** The durable pane association and claim (D5); the spill
    and its cursor (D6, D7a); reclaim visibility and resync (D7); one-active-client
    ownership (D8).
...
- **Durable, searchable command output.** The capture path that
  `ws_history_record.go:187` says does not exist. Building it means deciding who owns
  terminal bytes beside the renderer's scrollback.
...
1. **D6-a or D6-b** — lossless with the work stopping at the bound, or continuing work with
   a stated gap. AD-10 is amended either way, and the two are different products.
2. **The spill bound is deliberately left unset.** There is no defensible basis for a number
   in this repository: the only rate-like figure is the illustrative 256 KiB/s at
   `ring.go:20`, at which 8 MiB is about half a minute. We have no distribution for build
   output, TUI repaint volume, absence duration, concurrent live sessions or tolerable disk
   use. The number is chosen after those are measured — not borrowed from an unrelated
   limit.
