...
// delivery_unknown and is forgotten: writerBusy and inFlight clear without
// ever hearing back from the writer goroutine that is still blocked on the
// real write, which is what lets run's own exit condition become true. That
// writer goroutine is not stopped here — it keeps running, abandoned, until
// the Process's own Detach ends its connection (the peer closing the
// channel, or the pool closing a tainted one), and its eventual send on
// writeRes lands in that channel's one-slot buffer, read by nobody, which is
// what "never blocks" means for it.
//
