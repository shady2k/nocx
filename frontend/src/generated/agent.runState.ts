...
export interface AgentRunState {
  ...
  state:
    ...
    | 'interrupted'
  /**
   * The reason, present for failed runs and only when a cancelled turn has additional turn-specific detail: a sentence a person reads (for example, 'the endpoint's credential is unavailable'), never a Go error string. Cancelling a turn never reports or changes the host command.
   */
  error?: string
  /**
